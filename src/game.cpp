...
void set_piece_vals(map<char,int> &piece_vals, string max_player){
    short multiplier = 1; // By default max_player is black
    if(max_player == WHITE)
        multiplier = -1;
    // BLACK
...
vector<Move> game::get_all_moves(string player, bool legal, bool capture_only){
        ...
        if(get_player(this->game_board[i]) == player){
            ...
            for(auto j=temp.begin(); j != temp.end(); j++){
                ...
                if(this->is_capture(i, *j, player)){
                    captures.push_back(Move(i, *j, piece_vals[this->game_board[i]] - piece_vals[this->game_board[*j]]));
                }
        ...
        if(GameObj.game_board[i] != 'f'){
            ...
            if(test_bit(psuedo_target_areas.at(get_player(GameObj.game_board[i])), i) && tolower(GameObj.game_board[i]) == 'p'){
                if(!initial_rank)
                    ...
                    score += const_protection * multiplier;
            }
...
int quiescence_search(game GameObj, string player, int alpha, int beta){
    ...
    int stand_pat = heuristic(GameObj, player, player);
    int val;
    ...
    // Delta pruning
    if(stand_pat < alpha - delta && stand_pat >  late_game_cutoff)
...
int negamax(game GameObj, string player, short depth, int alpha, int beta, bool null_move){
    if(depth == 0)
    ...
    if(!null_move && check_null_move(GameObj, player) && depth > R){
        int val = -negamax(GameObj, reverse_player(player), depth - R - 1, -beta, -beta + 20, true);
        if(val >= beta)
            return beta;
    }
    ...
    for(auto i = moves.begin();i != moves.end(); i++){
        tempObj = game(GameObj);
        tempObj.make_move(i->current, i->target, player, false, true);
        val = max(val, -negamax(tempObj, reverse_player(player), depth - 1, -beta, -alpha, null_move));
        alpha = max(val, alpha);
        if(alpha >= beta)
            return alpha;
    }
...
Move call_ai(game GameObj, string player, short depth){
    ...
    for(auto temp = moves.begin();temp != moves.end(); temp++){
        ...
        temp_val = -negamax(tempObj, reverse_player(player), depth - 1, alpha, beta, false);
